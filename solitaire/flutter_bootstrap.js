// Engine bootstrap.
//
// Adapted from canuckleSourceCode/web/flutter_bootstrap.js, which is the most
// battle-tested file in the family. Three deliberate differences:
//
...
//
// Flutter 3.44 emits only a self-unregistering cleanup worker, so there is no
...
// Local development. A debug build is compiled with DDC and loads ~800
// separate module scripts, so first paint takes far longer than a release
// build does -- long enough on a cold start or a modest machine to trip the
// 60s error UI and show a developer a failure that is not real.
//
// So on localhost: no fatal-error timeout, and no ad tag. Neither belongs in
// a dev session, and the honest signal there is the Flutter console, not a
// "couldn't load the game" panel.
const IS_LOCAL_DEV =
...
function showFatalError() {
  if (typeof window.__solShowError === 'function') window.__solShowError();
}

function hideLoadingScreen() {
  if (typeof window.__solHideLoading === 'function') window.__solHideLoading();
}
...
async function runEntrypoint(engineInitializer) {
  ...
  try {
    ...
    // Inject the ad tag only after the app is running, plus a grace period.
    // The ad's own canvas/WebGL contexts can otherwise starve CanvasKit's
    // during the critical startup window, right as the first-launch how-to
    // overlay is painting.
    if (!IS_LOCAL_DEV) {
      setTimeout(function () {
...
function startFallbackLoad() {
  ...
  try {
    _flutter.loader.load({
      ...
      // revision -- and breaks as a blank canvas with no JS error the moment
      // one of them is upgraded and the other is not.
...
// deploy_to_site.ps1 does not build with --wasm.
try {
  _flutter.loader.load({
